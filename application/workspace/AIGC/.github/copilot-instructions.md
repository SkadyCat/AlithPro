...
本工作区可使用 `web_search` 工具进行互联网检索，获取最新的 AIGC、模型、工具链等信息。

使用场景：
- 查询最新的 AIGC 技术趋势、模型发布、工具更新
- 检索 Civitai 模型信息和使用策略
- 获取 ComfyUI 插件、节点、工作流的最新文档

可用的 MCP 搜索工具（来自 mrkrsl/web-search-mcp）：
- `get-web-search-summaries`：快速获取搜索结果摘要和链接，推荐作为第一步
- `full-web-search`：跟踪结果链接并提取完整页面内容，适合需要文章正文的场景
- `get-single-web-page-content`：已知 URL 时直接提取单个页面内容

推荐流程：先用 `get-web-search-summaries` 获取候选来源 → 审查 URL 和标题 → 用 `get-single-web-page-content` 提取具体页面

网络代理配置：
- 本机代理端口：`127.0.0.1:7897`
- 访问外网资源（如 Civitai、GitHub）时应通过代理

## AIGC 应用

本工作区部署了 ComfyUI 作为核心 AIGC 图像生成引擎。

关键路径：
- ComfyUI 部署目录：`application/ComfyUI/`
- 启动脚本：`application/start-comfyui.bat`
- 部署脚本：`application/setup-comfyui.ps1`
- Civitai 模型下载指南：`doc/aigc-civitai-guide.md`
- ComfyUI 默认端口：`http://127.0.0.1:8188`
