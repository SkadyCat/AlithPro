...
- Unless a task explicitly says otherwise, `MagicWorld` refers to the external project path `G:\GameExPro3\MagicWorld`.
...
## MagicWorld WBP Workflow

- `G:\GameExPro3\MagicWorld\McpFlask\app.py` is the HTTP bridge into Unreal editor automation.
- The actual Widget Blueprint creator is `G:\GameExPro3\MagicWorld\Plugins\AIWBPEditor\Source\AIWBPEditor\Public\AIWBPEditorLibrary.h` → `CreateWBPFromJson(...)`, which delegates to `FAIWBPImporter::ImportFromJsonToPath(...)`.
- `FAIWBPImporter` still consumes AIUI `schema_version = 2.0` JSON, not `.session` directly. Its core logic lives in:
  - `...Private\AIWBPImporter.cpp`
  - `BuildNode(...)`
  - `ApplySlotAuto(...)`
  - `ImportFromJsonToPath(...)`
- Therefore, when starting from UIEditor `.session`, prefer the bridge route instead of modifying importer input format first:
  - `GET /ue/create-wbp-from-session?session={file}.session&template={name}`
- Canonical `.session` source of truth is `application\canvas-editor\public\app.js` `sessionData()` and the spec file `application\canvas-editor\data\docs\specs\session-wbp-roundtrip-protocol.md`; `widgets` is canonical and `boxes` is compatibility only.
- Important path mismatch:
  - canvas-editor saves working sessions under `application\canvas-editor\data\docs\sessions\*.session`
  - `McpFlask` `/ue/create-wbp-from-session` currently reads `application\uieditor\template\*.session`
  - so before creating a WBP from the "current session", either sync/copy the file into `application\uieditor\template` or extend `McpFlask` to read the docs/sessions location directly.
- The bridge route converts UIEditor session data into AIUI JSON at:
  - `G:\GameExPro3\AIUI\umg\__session_bridge__{template}.json`
  and then calls `ue.AIWBPEditorLibrary.create_wbp_from_json(...)`.
- Legacy direct route remains:
  - `GET /ue/create-wbp?template={name}`
  which reads `G:\GameExPro3\AIUI\umg\{name}.json`.
- Recommended order for roundtrip work:
  1. Edit or export `.session`
  2. Call `/ue/create-wbp-from-session`
  3. If needed, call `/ue/export-wbp-meta`
  4. If needed, call `/ue/export-wbp-session`
- Prefer `POST http://127.0.0.1:5001/ue/create-wbp-from-session` with JSON body `{ session, template }` over browser GET when automating, to avoid escaping and browser/CORS issues.
- Importer limitation to remember: `FAIWBPImporter::ApplySlotAuto(...)` still recomputes tree-node offsets and hard-sets alignment to `(0,0)`, so exact preservation of stretch-anchor right/bottom margins or non-zero alignment may require importer-side C++ changes.
- For Unreal-side verification and debugging, use the project log directory `G:\GameExPro3\MagicWorld\Saved\Logs`.
  - Primary file: `G:\GameExPro3\MagicWorld\Saved\Logs\MagicWorld.log`
  - Use it to confirm whether `McpFlask` queued work, `AIWBPEditorLibrary.create_wbp_from_json(...)`, and importer-side execution actually completed, so the `.session -> bridge JSON -> WBP` flow is closed-loop verifiable.
- Only modify `AIWBPImporter.cpp` when the bridge route cannot express required widget data or when importer-side support for a new widget/slot behavior is genuinely missing.
